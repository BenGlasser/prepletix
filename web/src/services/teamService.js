...
export class TeamService {
    ...
    try {
      ...
      for (const teamId of teamIds) {
        try {
          ...
          const actualTeamId =
            typeof teamId === "string" ? teamId : teamId?.teamId || teamId?.id;
          console.log("🍕 TeamService: Processing team ID:", {
            original: teamId,
            actual: actualTeamId,
            type: typeof actualTeamId,
          });
          ...
          console.warn(`🍕 TeamService: Failed to load team`, {
            teamId,
            error: error.message,
          });
        }
      ...
      console.log(
        "🔄 TeamService: Falling back to legacy method - coach profile may not exist yet"
      );
      console.log(
        "💡 To fix this permanently, run the coach-centric migration in Settings"
      );
  ...
  static async addCoachToTeam(teamId, coachData) {
    try {
      console.log("🤝 TeamService: Adding coach to team via CoachService:", {
        teamId,
        coachData,
      });
  ...
  static async removeCoachFromTeam(teamId, coachUid) {
    try {
      console.log(
        "❌ TeamService: Removing coach from team via CoachService:",
        { teamId, coachUid }
      );
  ...
  static async getInvitationByCode(invitationCode) {
    try {
      console.log(
        "🔍 TeamService: Looking up invitation code:",
        invitationCode
      );
      ...
      console.log(
        "🔍 TeamService: Found",
        snapshot.size,
        "unused invitations matching code"
      );

      if (snapshot.empty) {
        console.log(
          "❌ TeamService: No unused invitations found for code:",
          invitationCode
        );
        return null;
      }
  ...
  static async applyInvitation(invitationCode, coachData) {
    try {
      ...
      const updatedCoach = await CoachService.addCoachToTeam(
        coachData.uid,
        invitation.teamId,
        "assistant"
      );
