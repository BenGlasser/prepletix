...
export default function DrillLibrary() {
  ...
  const loadDrills = useCallback(async () => {
    ...
    try {
      ...
      const drillList = allDrills.filter(
        (drill) => drill.isGlobal || drill.teamId === currentTeam.id
      );
  ...
  const filteredDrills = drills.filter((drill) => {
    ...
    if (
      filters.name &&
      !drill.name.toLowerCase().includes(filters.name.toLowerCase())
    ) {
      return false;
    }
    ...
    if (
      ...
      !filters.category.includes(drill.category)
    ) {
      return false;
    }
    ...
    if (
      ...
      !filters.skillLevel.includes(drill.skillLevel)
    ) {
      return false;
    }
  ...
  const handleAddDrill = async () => {
    try {
      ...
      // Add to local state
      const updatedDrills = [...drills, newDrill].sort((a, b) =>
  ...
  const handleFieldSave = async (fieldName) => {
    ...
    try {
      const updatedDrill = new Drill({
        ...selectedDrill,
        [fieldName]: fieldName.includes("Players")
          ? newValue
            ? parseInt(newValue)
            : null
          : newValue,
      ...
      await updateDoc(
        doc(db, "drills", selectedDrill.id),
        updatedDrill.toFirestore()
      );

      // Update local state
      selectedDrill[fieldName] = fieldName.includes("Players")
        ? newValue
          ? parseInt(newValue)
          : null
        : newValue;
      ...
      const updatedDrills = drills.map((d) =>
        d.id === selectedDrill.id ? { ...selectedDrill } : d
      );
      setDrills(updatedDrills);
  ...
  const handleAddEquipment = async () => {
    if (
      equipmentInput.trim() &&
      !selectedDrill.equipmentNeeded.includes(equipmentInput.trim())
    ) {
      const newEquipment = [
        ...selectedDrill.equipmentNeeded,
        equipmentInput.trim(),
      ];
  ...
  const handleRemoveEquipment = async (index) => {
    const newEquipment = selectedDrill.equipmentNeeded.filter(
      (_, i) => i !== index
    );
        ...
        <div className="flex justify-center items-center h-64">
          <div className="text-center">
            ...
            <div className="text-sm text-gray-400">
              Please select or create a team to view drills.
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 mt-1">
                Manage your coaching drills and exercises
              </p>
            </div>
                      ...
                      {filteredDrills.map((drill) => (
                        <tr
                          ...
                          className={`hover:bg-gray-50 dark:hover:bg-gray-700/30 cursor-pointer transition-colors duration-200 ${
                            selectedDrill && selectedDrill.id === drill.id
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      {editingFields.description ? (
                        <textarea
                          ...
                          onKeyDown={(e) =>
                            handleFieldKeyDown(e, "description")
                          }
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {selectedDrill.equipmentNeeded &&
                          selectedDrill.equipmentNeeded.length > 0 && (
                            <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                              {selectedDrill.equipmentNeeded.map(
                                (equipment, index) => (
                                  <li
                                    key={index}
                      ...
                      <div>
                        ...
                        {editingFields.minPlayers ? (
                          <input
                            ...
                            onKeyDown={(e) =>
                              handleFieldKeyDown(e, "minPlayers")
                            }
                        ...
                        {editingFields.maxPlayers ? (
                          <input
                            ...
                            onKeyDown={(e) =>
                              handleFieldKeyDown(e, "maxPlayers")
                            }
